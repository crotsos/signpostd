 ...
 * *)
  ...
  let rec filter_outgoing_synack_packet controller dpid evt =
    try_lwt
      let (pkt, port, buffer_id) = match evt with 
        | OC.Event.Packet_in(port, buffer_id, pkt, _) ->
                (pkt,port,buffer_id)
      ...
      let m = OP.Match.parse_from_raw_packet port pkt in
        ...
        * setup yet *)
      let flags = Tcp.get_tcp_flags pkt in
        match (flags.Tcp.syn, flags.Tcp.ack) with 
          | (true, true) -> begin
              ...
              lwt _ = Sp_controller.unregister_handler m filter_outgoing_synack_packet in 
              let pkt = OP.Flow_mod.create m 0L OP.Flow_mod.ADD
