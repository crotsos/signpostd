...
module Checksum = struct 
  let ones_complement bits offset len = 
    ...
    let res = add bits offset len in 
      if (res > 0xffff) then (
        let res = ((lnot ((res land 0xffff) + (res lsr 16))) land 0xffff) in 
          ( ((res land 0xff00) lsr 8) + ((res land 0xff) lsl 8) )
      ) else (
        let res = ((lnot res) land 0xffff) in
          ( ((res land 0xff00) lsr 8) + ((res land 0xff) lsl 8) )
      )
end
...
let get_tcp_packet_payload bits =
  ...
  let bits = shift bits (ip_len * 4) in
  let tcp_len = ((get_tcpv4_dataoff bits) land 0xf0) lsr 4 in
...
let gen_server_syn data new_isn local_mac gw_mac 
  ...
  let tcp_csm_buf = Cstruct.create 
  ...
  let _ = Cstruct.blit bits 0 tcp_csm_buf sizeof_pseudo_header 
            (Cstruct.len bits) in 
...
let gen_tcp_packet isn ack src_mac dst_mac src_ip dst_ip 
  ...
  let ret = Cstruct.create 2048 in
  ...
  * additional buffer *)
  ...
  let _ = set_pseudo_header_len tcp_csm_hdr 
            (20 + (Cstruct.len data)) in 
  ...
  let _ = set_tcpv4_checksum bits 0 in
(*  let _ = Cstruct.blit data 0 tcp_csm_hdr sizeof_tcpv4 
            (Cstruct.len data) in *)
  let _ = Cstruct.blit data 0 bits sizeof_tcpv4 
            (Cstruct.len data) in
